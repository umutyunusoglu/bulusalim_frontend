...
final ProviderFamily<FeedRepository, FeedType> feedRepositoryProvider =
    Provider.family<FeedRepository, FeedType>((
      ref,
      feedType,
    ) {
      // Pull infrastructure dependencies from get_it. Sources are
      // instantiated inline because their lifecycle is tied 1:1 to the
      // repository — no value in exposing them as separate providers.
...
final StreamProviderFamily<List<FeedEntity>, FeedType> feedStreamProvider =
    StreamProvider.family<List<FeedEntity>, FeedType>((
      ref,
      feedType,
    ) {
